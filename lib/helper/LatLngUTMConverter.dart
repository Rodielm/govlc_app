...
toLatLon(double easting, double northing, int zoneNum, String zoneLetter,
  ...
  if (zoneLetter.isEmpty && northern == null) {
    throw ('either zoneLetter or northern needs to be set');
  } else if (zoneLetter.isNotEmpty && northern != null) {
    throw ('set either zoneLetter or northern, but not both');
  }

  if (strict) {
    if (easting < 100000 || 1000000 <= easting) {
      throw ('easting out of range (must be between 100 000 m and 999 999 m)');
    }
    if (northing < 0 || northing > 10000000) {
      throw ('northing out of range (must be between 0 m and 10 000 000 m)');
    }
  }
  if (zoneNum < 1 || zoneNum > 60) {
    throw ('zone number out of range (must be between 1 and 60)');
  }
  if (zoneLetter.isNotEmpty) {
    ...
    northern = zoneLetter.codeUnitAt(0) >= 'N'.codeUnitAt(0);
  }
